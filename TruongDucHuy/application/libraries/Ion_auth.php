...
class Ion_auth
{
	...
	public function is_lecturer($id = FALSE)
	{
		...
		return $this->ion_auth_model->in_group($lecturer_group, $id);
	}
	public function is_parent($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_parent');

		$parent_group = $this->config->item('parent_group', 'ion_auth');

		return $this->ion_auth_model->in_group($parent_group, $id);
	}
